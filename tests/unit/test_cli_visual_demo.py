...
def test_demo_command_relays_visual_output_and_summary_json(
    ...
    def run_demo(
        ...
        *,
        ...
        payload = {"status": "ok", "cleanup_preview_left_bucket_state_unchanged": True}
...
def test_demo_command_exits_non_zero_when_summary_reports_error(
    ...
    monkeypatch.setattr(os, "environ", base_env)
    monkeypatch.setattr(cli_module, "configure_logging", lambda _: Path("/tmp/s3-archiver.log"))
...
def test_run_visual_demo_reports_bucket_story_and_cleanup_preview(
    ...
    def archive_runner(_settings: AppSettings, _log_file: Path) -> dict[str, object]:
    ...
    def cleanup_runner(_settings: AppSettings, _log_file: Path) -> dict[str, object]:
        return {
            "status": "ok",
            "cleanup_preview": {
    ...
    lines: list[str] = []
    monkeypatch.setattr(demo_module, "run_health_check", lambda *_args: FakeReport())
    monkeypatch.setattr(demo_module, "build_s3_client", lambda _location: object())
...
@pytest.mark.unit()
def test_visual_demo_utc_now_returns_utc_datetime() -> None:
    assert demo_module._utc_now().tzinfo is UTC
